...
internal class CustomScopeWithoutModulesTest {

    @Test
    @ResourceLocation("with_scope_without_modules")
    @DisplayName(
        "Project with single project profile with a custom scope with only the " +
            "needed files inside .idea folder"
    )
    fun testAny(
        ...
        val expected = xml("checkstyle") {
            ...
            "file" {
                ...
                "error" {
                    ...
                    attribute("message", "Class is not instantiated.")
