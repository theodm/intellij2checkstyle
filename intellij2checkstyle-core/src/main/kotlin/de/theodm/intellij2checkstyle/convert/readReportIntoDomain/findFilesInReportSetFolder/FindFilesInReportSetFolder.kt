package de.theodm.intellij2checkstyle.convert.readReportIntoDomain.findFilesInReportSetFolder
...
internal fun findFilesInReportSetFolder(
    ...
    if (!Files.exists(reportSetFolderPath)) {
        throw ReportSetFolderPathDoesNotExistException(
            reportSetFolderPath.toString()
        )
    }

    if (!Files.isDirectory(reportSetFolderPath)) {
        throw ReportSetFolderPathExistsButIsNoDirectoryException(
            reportSetFolderPath.toString()
        )
    }
    ...
    if (!Files.exists(descriptionFilePath)) {
        throw DescriptionFilePathDoesNotExistException(
